...
BEGIN
  IF OLD.is_draft = TRUE AND NEW.is_draft = FALSE THEN
    ...
    SELECT routing_mode, border_keywords INTO v_routing_mode, v_border_keywords
    ...
    IF v_routing_mode = 'sequential' THEN
      ...
      END IF;
    ELSE
      IF v_border_keywords IS NOT NULL AND array_length(v_border_keywords, 1) > 0 THEN
        border_kws := v_border_keywords;
      END IF;
