...
BEGIN
  IF OLD.is_draft = TRUE AND NEW.is_draft = FALSE THEN
    ...
    IF v_routing_mode = 'sequential' THEN
      ...
      END IF;
    ELSE
      -- Smart mode: use ONLY configured keywords, no hardcoded fallback
      -- If no keywords configured, treat all items as no-border (go to first production station)
      IF border_kws IS NULL OR array_length(border_kws, 1) IS NULL OR array_length(border_kws, 1) = 0 THEN
        -- No keywords = no border detection, send to first production station
        SELECT id INTO first_station_id
        FROM kds_stations
        WHERE is_active = TRUE AND station_type != 'order_status' AND tenant_id = NEW.tenant_id
        ORDER BY sort_order ASC LIMIT 1;
      ...
      ELSE
        ...
        FOR item_record IN
          SELECT oi.id, oi.notes,
                 ...
                 COALESCE((SELECT string_agg(lower(sie.option_name), ' ') FROM order_item_sub_items si JOIN order_item_sub_item_extras sie ON sie.sub_item_id = si.id WHERE si.order_item_id = oi.id AND sie.kds_category = 'border'), '') as border_extras_text
          FROM order_items oi
...
BEGIN
  ...
  IF (order_is_draft = FALSE AND NEW.current_station_id IS NULL) THEN
    ...
    IF v_routing_mode = 'sequential' THEN
      ...
      END IF;
    ELSE
      -- Smart mode: use ONLY configured keywords
      IF border_kws IS NULL OR array_length(border_kws, 1) IS NULL OR array_length(border_kws, 1) = 0 THEN
        -- No keywords = no border detection
        SELECT id INTO first_station_id
        FROM kds_stations WHERE is_active = TRUE AND station_type != 'order_status' AND tenant_id = order_tenant_id
        ORDER BY sort_order ASC LIMIT 1;
