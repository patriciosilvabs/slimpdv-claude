...
-- Update the apply_stock_movements_for_order function to check output_ingredient_id
...
BEGIN
  ...
  LOOP
    ...
    IF item_record.product_id IS NOT NULL THEN
      ...
      LOOP
        -- CHECK: does this recipe have an output_ingredient_id?
        SELECT r.output_ingredient_id, r.expected_yield
        INTO v_output_ingredient_id, v_expected_yield
        FROM public.recipes r
        WHERE r.id = recipe_record.recipe_id;

        IF v_output_ingredient_id IS NOT NULL THEN
          -- Deduct from the intermediate ingredient instead of raw materials
          SELECT i.id, i.current_stock, i.unit INTO ingredient_record
          FROM public.ingredients i
          WHERE i.id = v_output_ingredient_id AND i.tenant_id = order_record.tenant_id;

          IF ingredient_record.id IS NOT NULL THEN
            deduction_quantity := item_record.quantity * recipe_record.quantity_multiplier;
            IF ingredient_record.unit = 'kg' THEN
              deduction_quantity := deduction_quantity / 1000.0;
            END IF;

            INSERT INTO public.stock_movements (ingredient_id, movement_type, quantity, previous_stock, new_stock, tenant_id, order_id, notes)
            VALUES (v_output_ingredient_id, 'exit', deduction_quantity, COALESCE(ingredient_record.current_stock,0), GREATEST(0, COALESCE(ingredient_record.current_stock,0) - deduction_quantity), order_record.tenant_id, _order_id, 'Baixa automática - Produto intermediário');

            UPDATE public.ingredients SET current_stock = GREATEST(0, COALESCE(current_stock,0) - deduction_quantity), updated_at = now() WHERE id = v_output_ingredient_id;
          END IF;
        ELSE
          -- Original behavior: deduct raw ingredients proportionally
          FOR recipe_ing IN
    ...
    LOOP
      ...
      LOOP
        ...
        INTO v_output_ingredient_id, v_expected_yield
        FROM public.recipes r
        WHERE r.id = cor_record.recipe_id;
        ...
        IF v_output_ingredient_id IS NOT NULL THEN
          ...
          IF ingredient_record.id IS NOT NULL THEN
            deduction_quantity := item_record.quantity * cor_record.quantity_multiplier;
            IF ingredient_record.unit = 'kg' THEN
              deduction_quantity := deduction_quantity / 1000.0;
            END IF;

            INSERT INTO public.stock_movements (ingredient_id, movement_type, quantity, previous_stock, new_stock, tenant_id, order_id, notes)
            VALUES (v_output_ingredient_id, 'exit', deduction_quantity, COALESCE(ingredient_record.current_stock,0), GREATEST(0, COALESCE(ingredient_record.current_stock,0) - deduction_quantity), order_record.tenant_id, _order_id, 'Baixa automática - Produto intermediário Complemento');

            UPDATE public.ingredients SET current_stock = GREATEST(0, COALESCE(current_stock,0) - deduction_quantity), updated_at = now() WHERE id = v_output_ingredient_id;
          END IF;
        ELSE
          SELECT COALESCE(SUM(ri.quantity), 0)
          INTO recipe_total_quantity
          FROM public.recipe_ingredients ri
          WHERE ri.recipe_id = cor_record.recipe_id
            AND ri.tenant_id = order_record.tenant_id;

          IF recipe_total_quantity <= 0 THEN
            CONTINUE;
          END IF;
          ...
          LOOP
            ...
            IF ingredient_record.id IS NOT NULL THEN
              deduction_quantity := item_record.quantity * recipe_ing.recipe_quantity * (cor_record.quantity_multiplier / recipe_total_quantity);
              IF ingredient_record.unit = 'kg' THEN
                deduction_quantity := deduction_quantity / 1000.0;
              END IF;
    ...
    LOOP
      ...
      LOOP
        ...
        IF v_output_ingredient_id IS NOT NULL THEN
          ...
          IF ingredient_record.id IS NOT NULL THEN
            deduction_quantity := item_record.quantity * (cor_record.quantity_multiplier / flavor_count);
            IF ingredient_record.unit = 'kg' THEN
              deduction_quantity := deduction_quantity / 1000.0;
            END IF;

            INSERT INTO public.stock_movements (ingredient_id, movement_type, quantity, previous_stock, new_stock, tenant_id, order_id, notes)
            VALUES (v_output_ingredient_id, 'exit', deduction_quantity, COALESCE(ingredient_record.current_stock,0), GREATEST(0, COALESCE(ingredient_record.current_stock,0) - deduction_quantity), order_record.tenant_id, _order_id, 'Baixa automática - Produto intermediário Sabor');
        ...
        ELSE
          ...
          LOOP
            ...
            IF ingredient_record.id IS NOT NULL THEN
              deduction_quantity := item_record.quantity * ((recipe_ing.recipe_quantity * cor_record.quantity_multiplier) / recipe_total_quantity / flavor_count);
              IF ingredient_record.unit = 'kg' THEN
                deduction_quantity := deduction_quantity / 1000.0;
              END IF;
