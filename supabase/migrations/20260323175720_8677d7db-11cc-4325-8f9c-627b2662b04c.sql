...
BEGIN
  ...
  -- Fetch oven time from the OVEN station (oven_expedite type), not from current station
  SELECT oven_time_minutes INTO v_oven_station_time
  FROM kds_stations
  ...
  v_oven_time := COALESCE(_oven_minutes, v_oven_station_time, 12);
