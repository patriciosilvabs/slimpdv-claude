...
/* ── Helpers reutilizados do SectorQueuePanel ── */
...
function WaiterItemCard({
  ...
  return (
    <div className={cn(
      ...
      {/* Sabores em destaque (igual ao SectorQueuePanel) */}
      {flavors.length > 0 && (
...
export function KdsWaiterServePanel({ items, stationId, stationColor, isLoading, darkMode }: Props) {
  // Apenas pedidos de mesa (dine_in)
  ...
  return (
    <div className={cn('flex flex-col h-full overflow-hidden', bg)}>
      ...
      >
        ...
        <span className={cn('font-bold text-lg', textPrimary)}>Passa-prato — Aguardando serviço</span>
        <Badge
