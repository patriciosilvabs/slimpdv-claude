...
content = r"""import { useMemo } from 'react';
...
/* Card individual por item — mesmo padrao visual do SectorQueuePanel */
function WaiterItemCard({
  ...
  return (
    <div className={cn(
      ...
      >
        <UtensilsCrossed className="h-3 w-3 mr-1 inline" />
        SERVIR
      </Badge>

      {/* Linha do cabecalho: ID + Mesa + badges + timer */}
      ...
      {/* Sabores em destaque — fonte grande igual ao SectorQueuePanel */}
      {flavors.length > 0 && (
        <div className={cn('text-4xl font-extrabold uppercase tracking-wide leading-tight', readableText)}>
          {flavors.map((f, i) => (
            <p key={i}>{f}</p>
          ))}
        </div>
      ...
      {/* Produto sem sabor — nome em destaque */}
      {flavors.length === 0 && (
        <div className={cn('text-2xl font-extrabold uppercase tracking-wide', readableText)}>
      ...
      {/* Botao Servir */}
      <Button
        className="w-full font-bold text-white gap-2 h-10 mt-auto"
        style={{ backgroundColor: stationColor || '#10B981' }}
...
# Verify file
out, err = run('head -5 /var/www/slimpdv/src/components/kds/KdsWaiterServePanel.tsx')
print("=== FIRST 5 LINES ===")
print(out)
if err:
    print("ERR:", err)

out, err = run('tail -10 /var/www/slimpdv/src/components/kds/KdsWaiterServePanel.tsx')
print("=== LAST 10 LINES ===")
print(out)
if err:
    print("ERR:", err)
